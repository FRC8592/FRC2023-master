...
public class LeftConeConeAuto extends BaseAuto {
    ...
    private SwerveTrajectory GP1_TO_A = generate(
        config
            .setStartVelocity(0.0)
            .setEndVelocity(2.0)
            .setReversed(true),
        GAME_PIECE_1.translate(0.25, -0.05),
        GAME_PIECE_1.translate(-3.0, 0.25)
    );

    private SwerveTrajectory A_TO_SCORE = generate(
        slowConfig
            .setStartVelocity(2.0)
            .setEndVelocity(0.0)
            .setReversed(true),
        GAME_PIECE_1.translate(-3.0, 0.25),
        GAME_PIECE_1.translate(-4.2, 0.25)
    );
    ...
    public void initialize() {
        queue = new CommandQueue(
            ...
            new JointCommand( // TRACK grid and continue PRIME 4-bar
                new FollowerCommand(drive, vision, A_TO_SCORE.addVision().setAcceptanceRange(0.05)),
                new LiftCommand(elevator, Heights.PRIME)
            ),
            new JointCommand( // Lift elevator HIGH and score
                new LiftCommand(elevator, Heights.HIGH),
